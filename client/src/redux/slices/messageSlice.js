...
const messageSlice = createSlice({
  ...
  const receiverId = (receiver && (receiver.id || receiver._id)) || receiver;
      ...
      .addCase(markMessagesAsRead.fulfilled, (state, action) => {
        ...
        state.conversations = state.conversations.map((conversation) => {
          if ((conversation.user && (conversation.user.id || conversation.user._id)) === action.payload.userId) {
        ...
        if (state.messages[action.payload.userId]) {
          state.messages[action.payload.userId] = state.messages[action.payload.userId].map((message) => {
            if ((message.sender && (message.sender.id || message.sender._id)) === action.payload.userId && !message.isRead) {
...
export const { clearMessageError, setCurrentConversation, clearMessages } = messageSlice.actions;
