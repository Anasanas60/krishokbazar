...
const ProfilePage = () => {
  ...
  const handleProfileImageChange = async (e) => {
    ...
    try {
      const token = (window.__REDUX__ && window.__REDUX__.getState && window.__REDUX__.getState().auth?.token) || localStorage.getItem('token');
      const config = {
        headers: {
      ...
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/uploads/profile`, uploadForm, config);
      if (res.data && res.data.data && res.data.data.url) {
        setUserForm((prev) => ({ ...prev, profilePicture: res.data.data.url }));
      }
    } catch (err) {
          ...
          <form onSubmit={handleUserSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                >
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="w-16 h-16 rounded-full overflow-hidden bg-gray-100">
                          {userForm.profilePicture ? (
                            <img src={userForm.profilePicture} alt="Profile" className="w-full h-full object-cover" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center text-gray-400">{user?.name?.charAt(0) || <span />}</div>
                          )}
                        </div>
                  ...
                  <div className="relative">
                    <Input id="name" name="name" label="Full Name" value={userForm.name} onChange={handleUserChange} icon={<FaUser className="text-gray-400" />} />
                  </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <Input id="email" label="Email Address" value={user?.email} className="bg-gray-100 block w-full" disabled />
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  <Input id="phone" name="phone" label="Phone Number" value={userForm.phone} onChange={handleUserChange} icon={<FaPhone className="text-gray-400" />} />
                </div>
              </div>
            ...
            <Button type="submit" variant="primary" ariaLabel="Save profile" disabled={loading}>{loading ? 'Saving...' : 'Save Changes'}</Button>
          </form>
        </div>
      )}
            ...
            <div className="mb-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  <Checkbox id="acceptsPickup" label="Accepts Pickup Orders" checked={farmerForm.acceptsPickup} onChange={(v)=>setFarmerForm({...farmerForm, acceptsPickup: v})} />
                </div>

                <div className="flex items-center">
                  <Checkbox id="acceptsDelivery" label="Offers Delivery" checked={farmerForm.acceptsDelivery} onChange={(v)=>setFarmerForm({...farmerForm, acceptsDelivery: v})} />
                </div>

                {farmerForm.acceptsDelivery && (
                  <div>
                    <label htmlFor="deliveryRadius" className="block text-sm font-medium text-gray-700 mb-1">Delivery Radius (miles)</label>
                    <Input id="deliveryRadius" name="deliveryRadius" type="number" value={farmerForm.deliveryRadius} onChange={handleFarmerChange} className="w-32" min={0} />
                  </div>
                )}
              </div>
            </div>
